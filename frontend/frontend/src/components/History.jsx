...
const ORIGIN_LABELS = {
  "buy-on-secondary-market": "Secondary Market Purchase",
  "buy-on-primary-market": "Primary Market Purchase",
  "claim-token-from-activity-points": "Token Claim",
  "fantasy-item": "Item Purchase",
  "sell-on-secondary-market": "Secondary Market Sale",
  "booster-opening": "Booster Opening",
  "reward": "Reward",
};
      ...
      <div className="tx-card-mini-info">
        ...
        {price != null && (
          <p className="tx-card-mini-price">
            Paid: <span>{price}</span> {currencyLabel}
          </p>
        )}
      </div>
    </div>
...
function ProductDetail({ product, cardMap }) {
    ...
    return (
      <div className="tx-product">
        <p className="tx-product-type">
          {type === "secondary-market" ? "Card(s) purchased" : "Pack purchased"}
        </p>
        {listings.map((l, i) => {
          ...
          return card ? (
            ...
            <p key={i} className="tx-product-line">
              Card #{l.item?.number} — {l.price} {CURRENCY_LABELS[product.currency] ?? "coins"}
            </p>
          );
        })}
  ...
  if (type === "fantasy-item") {
    ...
    return (
      <div className="tx-product">
        <p className="tx-product-type">Item(s) purchased</p>
        {items.map((it, i) => {
          const title = it.data?.title?.FR ?? it.data?.title?.EN ?? "Item";
          return (
            <p key={i} className="tx-product-line">
              {title} × {it.quantity} — {it.price} each
            </p>
          );
        })}
        <p className="tx-product-amount">Total: {product.amount}</p>
  ...
  if (type === "claim-token-from-activity-points") {
    ...
    return (
      <div className="tx-product">
        <p className="tx-product-type">Token claim</p>
        <p className="tx-product-line">
          {p?.nbActivityPoints} activity pts → {p?.nbTokens} GNZ
        </p>
      </div>
    );
  }
...
export default function History({ data, cardMap }) {
  ...
  if (transactions.length === 0) return <p className="empty-state">No transactions found.</p>;
