...
function MatchRow({ match }) {
  ...
  const resultLabel = tied ? "DRAW" : won ? "WIN" : "LOSS";
        ...
        <tr className="match-expand-row">
          <td colSpan={6}>
            <div style={{ display: "flex", gap: 32, flexWrap: "wrap" }}>
              ...
              />
              <DeckDetail
                label={`${match.adversaryPseudo ?? "Adversary"}'s deck`}
...
export default function MatchHistory({ data }) {
  ...
  if (matches.length === 0) return <p className="empty-state">No matches found.</p>;
  ...
  return (
    <>
      <p className="section-meta">
        {matches.length} matches — {wins}W / {matches.length - wins}L
      </p>
