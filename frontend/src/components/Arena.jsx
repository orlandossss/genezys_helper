  ...
  return (
    <div>
      ...
      <p className="section-meta">
        Top {results.length} cartes · arène: {LEVEL_LABEL[arenaLevel]}
        {attr1 ? ` + ${attr1}` : ""}
        {attr2 ? ` + ${attr2}` : ""}
      </p>
...
function ArenaRanker({ listings }) {
  ...
  return (
    <div>
      ...
      <p className="section-meta">
        {ranked.length} cartes uniques · {combos.length} combinaisons d'arène évaluées
      </p>
...
const ARENA_TABS = ["Constructeur Arène", "Classeur Arène"];

export default function Arena({ data }) {
  const [active, setActive] = useState("Constructeur Arène");
  const listings = data?.data?.listings ?? [];

  if (listings.length === 0) {
    return <p className="empty-state">Récupérer les données pour charger les outils d'arène.</p>;
  }
  ...
  return (
    <div>
      ...
      {active === "Constructeur Arène" && <ArenaBuilder listings={listings} />}
      {active === "Classeur Arène" && <ArenaRanker listings={listings} />}
    </div>
  );
}
