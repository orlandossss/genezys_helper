        ...
        <>
          <div className="card-img-wrapper">
            {(fullImage || thumbnail) ? (
              ...
              <div className="card-img-placeholder">Pas d'image</div>
            )}
          </div>
            ...
            {floorPrice != null && (
              <div className="card-price-row">
                <p className="card-floor">
                  plancher <span className="card-floor-price">{floorPrice}</span>
                </p>
                {isFloor && <span className="card-floor-badge">plancher</span>}
              </div>
            )}
            {purchasePrice != null && (
              <p className="card-purchase-price">
                Payé: <span>{purchasePrice}</span> {currencyLabel}
              </p>
            )}
          </div>
          <p className="card-flip-hint">cliquer pour détails</p>
        </>
      ) : (
        <div className="card-detail">
          ...
          {purchasePrice != null && (
            <p style={{ color: "var(--amber)", fontSize: "0.72rem", fontFamily: "'Fira Code', monospace", marginBottom: 8, letterSpacing: "0.04em" }}>
              Payé: <span style={{ fontWeight: 700 }}>{purchasePrice}</span> {currencyLabel}
            </p>
          )}
          ...
          <p className="card-flip-hint">cliquer pour revenir</p>
        </div>
      )}
    </div>
  );
